...
abstract class Odin_Front_End_Form {
	...
	protected function process_buttons() {
		...
		if ( ! empty( $this->buttons ) ) {
			...
			$html .= '<button type="submit" class="btn btn-primary">' . __( 'Submit', 'comunidade-wordpress-br' ) . '</button>';
		}
	...
	protected function display_success_message() {
		...
		if ( isset( $_GET['success'] ) && 1 == $_GET['success'] ) {
			...
			if ( ! empty( $this->success ) ) {
				...
				$html .= '<p>' . __( 'Form submitted successfully!', 'comunidade-wordpress-br' ) . '</p>';
			}
			$html .= '</div>';
		}
			...
			foreach ( $this->fields as $fieldset ) {
				foreach ( $fieldset['fields'] as $field ) {
					...
					if ( $required && empty( $data[ $id ] ) ) {
						$errors[] = sprintf( __( '%s is required.', 'comunidade-wordpress-br' ), '<strong>' . $label . '</strong>' );
					}

					switch ( $type ) {
						case 'email':
							if ( ! is_email( $value ) )
								$errors[] = sprintf( __( '%s must be an email address valid.', 'comunidade-wordpress-br' ), '<strong>' . $label . '</strong>' );
							break;
