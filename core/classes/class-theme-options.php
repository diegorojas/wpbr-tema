...
class Odin_Theme_Options {
	...
	function scripts() {
		...
		if ( isset( $_GET['page'] ) && $this->id == $_GET['page'] ) {
			...
			wp_localize_script(
				...
				'odinAdminParams',
				array(
					'galleryTitle'  => __( 'Add images in gallery', 'comunidade-wordpress-br' ),
					'galleryButton' => __( 'Add in gallery', 'comunidade-wordpress-br' ),
					'galleryRemove' => __( 'Remove image', 'comunidade-wordpress-br' ),
					'uploadTitle'   => __( 'Choose a file', 'comunidade-wordpress-br' ),
					'uploadButton'  => __( 'Add file', 'comunidade-wordpress-br' ),
				)
			);
		}
	}
	...
	public function callback_upload( $args ) {
		...
		$html = sprintf( '<input type="text" id="%1$s" name="%2$s[%1$s]" value="%3$s" class="regular-text"%5$s /> <input class="button odin-upload-button" id="%1$s-button" type="button" value="%4$s" />', $id, $tab, $current, __( 'Select file', 'comunidade-wordpress-br' ), $this->build_field_attributes( $attrs ) );
	...
	public function callback_image_plupload( $args ) {
		...
		$html = '<div class="odin-gallery-container">';
					...
					if ( $attachments ) {
						foreach ( $attachments as $attachment_id ) {
							$html .= sprintf( '<li class="image" data-attachment_id="%1$s">%2$s<ul class="actions"><li><a href="#" class="delete" title="%3$s">X</a></li></ul></li>',
								...
								__( 'Remove image', 'comunidade-wordpress-br' )
							);
						}
					}
				}
			...
			$html .= sprintf( '<p class="odin-gallery-add hide-if-no-js"><a href="#">%s</a></p>', __( 'Add images in gallery', 'comunidade-wordpress-br' ) );
		$html .= '</div>';
